// Copyright (c) 2012-2013 The Cryptonote developers111
...
#define ALLOY_MEMORY    (1 << 21) /* 2 MiB */
#define ALLOY_ITER      0x100000
#define ALLOY_MASK      0x1FFFF
...
void cryptonight_hash(const char* input, char* output, uint32_t len, int alloy) {
    ...
    size_t i, j;
    size_t memory = alloy ? ALLOY_MEMORY : MEMORY;
    size_t iterations = alloy ? ALLOY_ITER : ITER;
    size_t mask = alloy ? ALLOY_MASK : MASK;
